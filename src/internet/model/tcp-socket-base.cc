          ...
          && !m_persistEvent.IsRunning ())
        {
          ...
          if (tcpHeader.GetAckNumber () == m_txBuffer->HeadSequence ()
              && tcpHeader.GetAckNumber () < m_tcb->m_nextTxSequence)
            {
              ...
              Ptr<TcpOptionSack> sackBlock = m_txBuffer->CraftSackOption (tcpHeader.GetAckNumber (), available);
              if (sackBlock != 0)
                {
...
{
  NS_LOG_FUNCTION_NOARGS ();
  uint32_t unack = BytesInFlight (); // Number of outstanding bytes
  uint32_t win = Window ();          // Number of bytes allowed to be outstanding

  if (win < unack)
    {
      NS_LOG_DEBUG ("InFlight=" << unack << ", Win=" << win << " availWin=0");
      return 0;
    }

  NS_LOG_DEBUG ("InFlight=" << unack << ", Win=" << win << " availWin=" << win-unack);
  return win - unack;
}
...
TcpSocketBase::ReTxTimeout ()
{
  ...
  // Reset highTxMark and dupAckCount
  //m_tcb->m_highTxMark = m_txBuffer->HeadSequence ();
  m_dupAckCount = 0;
